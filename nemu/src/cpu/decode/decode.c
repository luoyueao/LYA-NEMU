...
static inline make_DopHelper(SI) {
  ...
  rtlreg_t temp;
  rtl_li(&temp, instr_fetch(eip, op->width)); //通过instr_fetch()函数读取，并存入寄存器temp
  rtl_sext(&temp, &temp, op->width);          //若立即数长度小于操作数长度，则进行符号扩展，否则不作修改  
  op->simm = temp;                            //将扩展后的结果存入op->simm
