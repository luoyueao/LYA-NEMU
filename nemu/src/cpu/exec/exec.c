...
opcode_entry opcode_table [512] = {
  ...
  /* 0xa4 */	IDEXW(E, ), EMPTY, EMPTY, EMPTY,
...
#ifdef CATCH_LOOP
  if(decoding.is_jmp) {                 //若需要跳转
    loop_count ++;                      //跳转次数+1
    if(loop_count > 1000000000) {       //当跳转次数大于某值，继续判断执行时间
      long loop_count_end = clock();
      Assert((loop_count_end - loop_count_start) < 60000000, "Encounter a dead loop!\n");
    }                                   //当执行时间也大于等于某值时，终止程序并输出信息
  }
#endif
